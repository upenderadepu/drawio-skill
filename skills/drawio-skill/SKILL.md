...
| `scripts/tfimports.py` · `k8simports.py` | The user wants to visualize **Terraform or Kubernetes infrastructure** — extracts the resource/object reference graph with each node resolved to its **official cloud icon** (AWS/Azure/GCP via the shape index, K8s kind icons) for autolayout |
